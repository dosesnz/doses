...
v13: event detection for the mailing list. Writes events.json describing anything
     worth emailing about: a price moving 1.5% or more, or a product appearing that
     has not been seen before. Also checks whether semaglutide or tirzepatide has
     become funded, but never emails on that - it opens a GitHub issue for a human,
     because announcing a Pharmac decision wrongly is worse than announcing it late.
...
Usage:
  ...
  python glp1_scraper_v13.py            # appends to data/prices.csv
  python glp1_scraper_v13.py --dry-run  # prints, doesn't write
...
KNOWN_PATH = os.path.join("data", "known_products.json")
MOVE_PCT = 1.5          # a price move worth emailing about
PHARMAC_SCHEDULE = "https://schedule.pharmac.govt.nz/latest/Schedule.pdf"


def _key(r):
    return "|".join([r["retailer"], r["drug"], r["strength"]])
...
def previous_prices():
    """Latest price per retailer+drug+strength from the existing CSV."""
    prev = {}
    ...
    try:
        with open(OUT_PATH, newline="", encoding="utf-8") as f:
            for row in csv.DictReader(f):
                if not row.get("price_nzd") or (row.get("note") or "").startswith("ERROR"):
                    continue
                k = "|".join([row["retailer"], row["drug"], row["strength"]])
                try:
                    price = float(row["price_nzd"])
                except ValueError:
                    continue
                cur = prev.get(k)
                if cur is None or row["date"] > cur["date"]:
                    prev[k] = {"date": row["date"], "price": price}
    except Exception as e:
        print(f"[events] could not read history: {e}", file=sys.stderr)
    return prev


def check_pharmac_funding():
    """Is semaglutide or tirzepatide named in the current Pharmac Schedule?

    Returns a dict of molecule -> bool, or None if the check could not run. This
    never triggers an email: a wrong 'Wegovy is funded now' is far worse than a
    late one, so a change here raises an issue for a person to confirm.
    """
    try:
        r = requests.get(PHARMAC_SCHEDULE, headers=HEADERS, timeout=60)
        r.raise_for_status()
        blob = r.content.lower()
        return {m: (m.encode() in blob) for m in ("semaglutide", "tirzepatide")}
    except Exception as e:
        print(f"[events] Pharmac check skipped: {e}", file=sys.stderr)
        return None


def detect_events(rows):
    """Compare this run against history and record anything worth sending."""
    prev = previous_prices()
    known = set(_load_json(KNOWN_PATH, []))
    first_run = not known and not prev

    moves, arrivals = [], []
    seen_now = set()

    for r in rows:
        if not r["price_nzd"] or (r["note"] or "").startswith("ERROR"):
            continue
        k = _key(r)
        seen_now.add(k)
        price = float(r["price_nzd"])

        # New product. Skipped on a first run, when everything is new.
        if not first_run and k not in known:
            arrivals.append({"product": f"{r['drug']} {r['strength']}",
                             "retailer": r["retailer"], "price": price,
                             "url": r["source_url"]})

        # Price move past the threshold.
        old = prev.get(k)
        if old and old["price"] > 0:
            pct = (price - old["price"]) / old["price"] * 100.0
            if abs(pct) >= MOVE_PCT:
                moves.append({"product": f"{r['drug']} {r['strength']}",
                              "retailer": r["retailer"],
                              "was": round(old["price"], 2), "now": round(price, 2),
                              "pct": round(pct, 2),
                              "direction": "down" if pct < 0 else "up",
                              "url": r["source_url"]})

    # Pharmac: compare against the last recorded answer, two days running before
    # it counts as a real change.
    last = _load_json(EVENTS_PATH, {})
    funding = check_pharmac_funding()
    funding_state = last.get("pharmac_state") or {}
    pending = last.get("pharmac_pending") or {}
    pharmac_change = None
    if funding is not None:
        if not funding_state:
            funding_state = funding          # first observation, nothing to compare
        else:
            changed = {m: v for m, v in funding.items() if funding_state.get(m) != v}
            if changed:
                if pending == changed:       # same change seen yesterday, trust it
                    pharmac_change = changed
                    funding_state = funding
                    pending = {}
                else:
                    pending = changed        # hold one day before believing it
            else:
                pending = {}

    events = {
        "generated": now_iso(),
        "email_worthy": bool(moves or arrivals),
        "moves": moves,
        "arrivals": arrivals,
        "pharmac_state": funding_state,
        "pharmac_pending": pending,
        "pharmac_change": pharmac_change,
    }
    ...
    with open(KNOWN_PATH, "w", encoding="utf-8") as f:
        json.dump(sorted(known | seen_now), f, indent=2)

    for m in moves:
        print(f"[event] {m['product']} at {m['retailer']}: {m['was']} -> {m['now']} ({m['pct']:+.1f}%)")
    for a in arrivals:
        print(f"[event] new product: {a['product']} at {a['retailer']}")
    if pharmac_change:
        print(f"[event] PHARMAC CHANGE (needs a human): {pharmac_change}")
    if not (moves or arrivals or pharmac_change):
        print("[event] nothing worth sending")
